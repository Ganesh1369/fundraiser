            ...
            <section class="relative min-h-[88vh] pt-24 pb-12 px-5 flex flex-col justify-center overflow-hidden"
                ...
                <div class="max-w-6xl mx-auto w-full grid lg:grid-cols-2 gap-10 lg:gap-12 items-center relative z-10">
                    ...
                    <div class="bg-white rounded-2xl p-6 lg:p-7 shadow-card w-full text-accent">
                        ...
                        <ng-container *ngIf="isTreeProject; else rupeeChips">
                            <div class="text-base font-bold mb-4">Choose how many trees</div>
                            <div class="rounded-xl border border-neutral-200 divide-y divide-neutral-100 mb-4 overflow-hidden">
                                <div *ngFor="let tier of treeTiers" class="flex items-center justify-between px-3 py-2 text-sm">
                                    <span class="font-semibold text-accent">{{ tier.range }}</span>
                                    <span class="text-neutral-600">{{ formatCurrency(tier.pricePerTree) }} / tree</span>
                                </div>
                            </div>
                            <button (click)="donate()"
                                class="w-full px-3 py-3.5 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-primary-500 transition-colors mb-3 inline-flex items-center justify-center gap-1.5">
                                Donate
                                <lucide-icon name="arrow-right" class="w-3.5 h-3.5"></lucide-icon>
                            </button>
                        </ng-container>
                        <ng-template #rupeeChips>
...
export class ProjectLandingComponent implements OnInit, OnDestroy {
    ...
    selectedAmount = 1000;
    // Tree-planting price tiers (shown on the ROOTS quick-donate card).
    readonly treeTiers = [
        { range: '1-5', pricePerTree: 1999 },
        { range: '6-10', pricePerTree: 1750 },
        { range: '11-20', pricePerTree: 1500 },
        { range: '21-50', pricePerTree: 1250 },
        { range: '50 above', pricePerTree: 1000 }
    ];
    /** True for the tree-planting project (ROOTS) — swaps rupee chips for tree pricing. */
    get isTreeProject(): boolean {
